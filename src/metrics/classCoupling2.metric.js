                    ...
                    innerPath.traverse({
                        NewExpression(deepPath) {
                            ...
                            */
                            ...
                            let calleeMethod = '_constructor' // Because this is new Class()
                            ...
                            for (const [filePath, classes] of Object.entries(state.result)) {
                                ...
                                if (classes[calleeClass]) {
                                    ...
                                    if(classes[calleeClass][calleeMethod]) {
                                        calleeFilepath = filePath;
                                    }
                                }
                            }

                            if (!state.result[state.currentFile][callerClass][callerMethod]['fan-out']) {
                                state.result[state.currentFile][callerClass][callerMethod]['fan-out'] = {};
                            }

                            if (!state.result[state.currentFile][callerClass][callerMethod]['fan-out'][calleeClass]) {
                                state.result[state.currentFile][callerClass][callerMethod]['fan-out'][calleeClass] = {}
                            }

                            if (!state.result[state.currentFile][callerClass][callerMethod]['fan-out'][calleeClass][calleeMethod]) {
                                state.result[state.currentFile][callerClass][callerMethod]['fan-out'][calleeClass][calleeMethod] = 0;
                            }

                            state.result[state.currentFile][callerClass][callerMethod]['fan-out'][calleeClass][calleeMethod]++;


                            if (!state.result[calleeFilepath][calleeClass][calleeMethod]['fan-in']) {
                                state.result[calleeFilepath][calleeClass][calleeMethod]['fan-in'] = {};
                            }

                            if (!state.result[calleeFilepath][calleeClass][calleeMethod]['fan-in'][callerClass]) {
                                state.result[calleeFilepath][calleeClass][calleeMethod]['fan-in'][callerClass] = {}
                            }

                            if (!state.result[calleeFilepath][calleeClass][calleeMethod]['fan-in'][callerClass][callerMethod]) {
                                state.result[calleeFilepath][calleeClass][calleeMethod]['fan-in'][callerClass][callerMethod] = 0;
                            }

                            state.result[calleeFilepath][calleeClass][calleeMethod]['fan-in'][callerClass][callerMethod]++;
                        },
